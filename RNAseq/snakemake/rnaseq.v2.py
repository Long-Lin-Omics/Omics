...
has_fastq2 = [("fastq2" in sample) for sample in all_samples.values()]
PAIRED = all(has_fastq2)

if any(has_fastq2) and not PAIRED:
    raise ValueError(
        "All samples must have the same libray layout: single-end or paired-end."
    )
...
onstart:
    print(
        f"[INFO] Detected sequencing mode: "
...
def get_fastq_path(sample, read):
    if read == "R1":
        return all_samples[sample]["fastq1"]
    return all_samples[sample]["fastq2"]


def raw_fastqc_reports(wildcards):
    return [
        f"{output_dir}/fastqc_raw/{sample}_{read}_fastqc.html"
        for sample in sample_names
        for read in READS
    ]


def trimmed_fastqc_reports(wildcards):
    return [
        f"{output_dir}/fastqc_trimmed/{sample}_{read}_trimmed_fastqc.html"
        for sample in sample_names
        for read in READS
    ]
...
rule_all_inputs = [
    f"{output_dir}/multiqc/multiqc_raw_fastq_report.html",
    f"{output_dir}/multiqc/multiqc_trimmed_fastq_report.html",
    f"/data/wade/linl7/{identifier}_BPM_normalised/hub.txt",
    expand(
        "{output_dir}/DEG_by_DESeq2_no_spike/{comparisons}/sample.list",
        comparisons=comparisons,
        output_dir=output_dir,
    ),
    f"{output_dir}/report/stat.tsv",
]

if SPIKEIN:
    rule_all_inputs += [
        f"{output_dir}/align/scaleFactor.overview.txt",
        expand(
...
if PAIRED:
    rule fastqc_raw:
        input:
            fastq1=lambda wc: all_samples[wc.sample]["fastq1"],
            fastq2=lambda wc: all_samples[wc.sample]["fastq2"]
        output:
            fastq1="{output_dir}/input/{sample}_R1.fastq.gz",
            fastq2="{output_dir}/input/{sample}_R2.fastq.gz",
            report1="{output_dir}/fastqc_raw/{sample}_R1_fastqc.html",
            report2="{output_dir}/fastqc_raw/{sample}_R2_fastqc.html"
        shell:
            r"""
            set -euo pipefail
            mkdir -p {output_dir}/input {output_dir}/fastqc_raw
            ln -sf {input.fastq1} {output.fastq1}
            ln -sf {input.fastq2} {output.fastq2}
            {scripts_folder}/RNAseq/softwares/fastqc {output.fastq1} {output.fastq2} -o {output_dir}/fastqc_raw/ -t 8
            """
    ...
    rule cutadapt:
        ...
        shell:
            ...
            {scripts_folder}/RNAseq/softwares/cutadapt -q 20 -m 20 \
                -a AGATCGGAAGAGC -A AGATCGGAAGAGC \
                -o {output.trimmed_fastq1} -p {output.trimmed_fastq2} \
                {input.fastq1} {input.fastq2} -j 8
            """
    ...
    rule fastqc_trimmed:
        ...
        output:
            ...
            "{output_dir}/fastqc_trimmed/{sample}_R2_trimmed_fastqc.html"
        shell:
            r"""
            set -euo pipefail
            mkdir -p {output_dir}/fastqc_trimmed
            {scripts_folder}/RNAseq/softwares/fastqc {input.trimmed_fastq1} {input.trimmed_fastq2} -o {output_dir}/fastqc_trimmed/ -t 8
            """
    ...
    rule STAR:
        ...
        shell:
            r"""
            set -euo pipefail
            {scripts_folder}/RNAseq/softwares/STAR --runThreadN 8 --genomeDir {genome} \
                --readFilesIn {input.trimmed_fastq1} {input.trimmed_fastq2} \
            ...
            && {scripts_folder}/RNAseq/softwares/java -Xmx90g -jar {scripts_folder}/RNAseq/softwares/picard.jar MarkDuplicates \
                I={output.sortbam} O=/dev/null M={output.dup_mets} REMOVE_DUPLICATES=false CREATE_INDEX=false
            """
    ...
    rule fastqc_raw:
        ...
        shell:
            ...
            mkdir -p {output_dir}/input {output_dir}/fastqc_raw
            ln -sf {input.fastq1} {output.fastq1}
            {scripts_folder}/RNAseq/softwares/fastqc {output.fastq1} -o {output_dir}/fastqc_raw/ -t 8
            """
    ...
    rule STAR:
        ...
        shell:
            ...
            set -euo pipefail
            {scripts_folder}/RNAseq/softwares/STAR --runThreadN 8 --genomeDir {genome} \
                --readFilesIn {input.trimmed_fastq1} \
                --outFileNamePrefix {params.out_prefix} \
                --readFilesCommand zcat \
                --outSAMtype BAM Unsorted \
                --quantTranscriptomeBan Singleend \
...
rule qualimap:
    ...
    shell:
        r"""
        set -euo pipefail
        {scripts_folder}/RNAseq/softwares/qualimap bamqc -nt 8 -bam {input.sortbam} -gff {gtf} -outdir {params.bamqc_output_prefix} --java-mem-size=90G && {scripts_folder}/RNAseq/softwares/qualimap rnaseq -bam {input.sortbam} -gtf {gtf} -outdir {params.rnaseq_output_prefix} --java-mem-size=90G
        """


rule multiqc_trimmed:
    input:
        trim=trimmed_fastqc_reports,
        qc=expand(
            "{output_dir}/align/{sample}.rnaseq.qualimap.report/rnaseq_qc_results.txt",
            sample=sample_names,
            output_dir=output_dir,
        )
    output:
...
rule normalize_scalefactors:
    ...
    output:
        expand(
            "{output_dir}/align/{sample}_scaleFactor.txt",
            sample=sample_names,
            output_dir=output_dir,
        )
    run:
        raw = {wildcard: float(open(f).read()) for wildcard, f in zip(sample_names, input)}
    ...
    run:
        with open(output[0], "w") as out:
            ...
            for s in sample_names:
                ...
                out.write(
                    s + "\t" + str(read_count) + "\t" + str(ecoli_count) + "\t" +
                    str(raw_factor) + "\t" + str(scale_factor) + "\n"
                )
...
rule ucsc_hub:
    input:
        unnormalized=expand(
            "{output_dir}/ucsc/{sample}_unnormalized.bw",
            sample=sample_names,
            output_dir=output_dir,
        ),
        bpm_normalized=expand(
            "{output_dir}/ucsc/{sample}_BPM_normalized.bw",
            sample=sample_names,
            output_dir=output_dir,
        ),
        spikeIn_normalized=expand(
            "{output_dir}/ucsc/{sample}_spikeIn_normalized.bw",
            sample=sample_names,
            output_dir=output_dir,
        ) if SPIKEIN else []
    output:
...
rule report_stat:
    input:
        ...
        dup_mets=expand(
            "{output_dir}/align/{sample}_dup_metrics.txt",
            output_dir=output_dir,
            sample=sample_names,
        )
    output:
...
rule deseq2:
    input:
        salmon_sf=lambda wildcards: expand(
            "{output_dir}/align/{case}.Aligned.toTranscriptome.salmon_quant/quant.sf",
            output_dir=output_dir,
            case=sorted(config["comparisons"][wildcards.comparisons]),
        )
    output:
    ...
    shell:
        ...
        for sf in {input.salmon_sf}; do
            ...
            cond=$(echo $sm | sed -E 's/_rep_[0-9]+$//')
